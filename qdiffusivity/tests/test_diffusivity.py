...
# ---------------------------------------------------------------------------
# LocalDiffusivityQKDE AnalysisBase
# ---------------------------------------------------------------------------
...
def test_diffusion_kde_dim_validation():
    ...
    with pytest.raises(ValueError):
        LocalDiffusivityQKDE(u.select_atoms("all"), dim=5)


def test_diffusion_kde_bandwidth_validation():
    u = _make_diffusion_universe(n_atoms=10, n_frames=3, Lz=10.0)
    with pytest.raises(ValueError):
        LocalDiffusivityQKDE(u.select_atoms("all"), bandwidth="bogus")


def test_diffusion_kde_kernel_validation():
    u = _make_diffusion_universe(n_atoms=10, n_frames=3, Lz=10.0)
    with pytest.raises(ValueError):
        LocalDiffusivityQKDE(u.select_atoms("all"), kernel="triangular")
...
def test_diffusion_kde_exposed_from_package():
    assert hasattr(qdiffusivity, "LocalDiffusivityQKDE")
...
def test_ito_correction_default_is_off():
    ...
    ag = u.select_atoms("all")
    kde = LocalDiffusivityQKDE(ag, dim=2, n_points=20, bandwidth=0.1)
    kde.run()
...
def test_ito_correction_does_not_affect_D_para():
    ...
    ag = u.select_atoms("all")
    kde_unc = LocalDiffusivityQKDE(ag, dim=2, n_points=30, bandwidth=0.1)
    kde_unc.run()
    kde_cor = LocalDiffusivityQKDE(
